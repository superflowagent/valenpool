      ...
      <div ref={containerRef} className="relative">
        <div ref={innerRef} className="flex items-center" style={{ willChange: 'transform' }}>
          <div ref={trackRef} className="flex">
            {logos.map((logo, i) => (
              <div key={`logo-${i}`} className={`w-75 h-42.5 md:w-82.5 md:h-47.5 rounded flex items-center justify-center shrink-0 ${i === 7 ? 'bg-black' : ''}`}>
          ...
          <div aria-hidden="true" className="flex">
            {logos.map((logo, i) => (
              <div key={`logo-dup-${i}`} className={`w-75 h-42.5 md:w-82.5 md:h-47.5 rounded flex items-center justify-center shrink-0 ${i === 7 ? 'bg-black' : ''}`}>
                <img src={logo.src} alt="" className="max-h-36 max-w-72 object-contain" />
              </div>
            ))}
          </div>
        </div>
