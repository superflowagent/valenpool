...
const ContactForm: React.FC = () => {
  ...
  const handleLocalityChange = useCallback(
    (value: string) => setForm((prev) => ({ ...prev, locality: value })),
    [],
  );
  const handlePoolTypeChange = useCallback(
    (value: string) => setForm((prev) => ({ ...prev, poolType: value })),
    [],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Validación básica -- mostrar mensaje inline en vez de alert
    if (!form.name || !form.phone || !form.locality || !form.poolType) {
      setResultMessage("Por favor completa los campos obligatorios.");
      return;
    }

    // Requiere llave de entorno para servicios externos
    const accessKey = (import.meta as { env?: { VITE_WEB3FORMS_KEY?: string } })
      ?.env?.VITE_WEB3FORMS_KEY;
    if (!accessKey) {
      setResultMessage(
        "El servicio de envío no está configurado. Contacta con el administrador.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setResultMessage(text || "Error al enviar el formulario");
        return;
      }
      ...
      if (import.meta.env.DEV) console.error("Error enviando formulario", err);
            ...
            >
              <div>
                ...
                <input
                  ...
                  required
                  aria-required="true"
                  className="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:border-primary focus:ring focus:ring-primary/30 h-10 pl-3"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows={4}
                  className="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:border-primary focus:ring focus:ring-primary/30 pl-3 py-2"
                />
              </div>
              ...
              <div>
                ...
                {(() => {
                  const isValid =
                    form.name && form.phone && form.locality && form.poolType;
                  return (
                    <button
                      type="submit"
